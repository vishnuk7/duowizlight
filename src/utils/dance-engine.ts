...
export class DanceEngine {
  ...
  public getBeatsMap(analysis: AudioAnalysis, id: string) {
    ...
    const highestRelativeLoudness = 100 + analysis.sections.reduce((result, section) => (result > section.loudness ? result : section.loudness), -Infinity);
  ...
  public translateBeatsToLights(beats: Beats, isPartyMode: boolean, alternateBrightness: boolean) {
    const lights = {
      delayMs: 500 / beats.beatsPerSec,
      colorSpace: getColorSpace(beats.key),
      brightness: Math.max(7, Math.round(beats.relativeLoudness * 0.6)),
    };
